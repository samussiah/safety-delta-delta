(function (global, factory) {
    ...
    (global = global || self, global.safetyDeltaDelta = factory(global.d3, global.webCharts));
    ...
    function initNotes() {
        ...
        this.wrap.insert('p', ':first-child').attr('class', 'record-note').style('text-align', 'center').style('font-weight', 'bold').text('Click a point to see details.');
        ...
        if (this.data.raw.length > 0) {
            ...
            this.tbody.selectAll('tr').style('background', 'none').style('border-bottom', '.5px solid black').each(function (row_d) {
                ...
                var cell = d3.select(this).select('td.spark').classed('minimized', true).text(''),
                    toggle = d3.select(this).select('td.toggle').html('&#x25BD;').style('cursor', 'pointer').style('color', '#999').style('vertical-align', 'middle'),
                    width = 100,
                    height = 25,
                    offset = 4,
    ...
    function addRegressionLine() {
        if (this.config.add_regression_line) {
            ...
            chart.svg.append('path').classed('regressionLine', true).datum([min_xy, max_xy]).attr('d', line).attr('stroke', 'black').attr('stroke-dasharray', '3,5');
    ...
    function styles() {
        var styles = ['#safety-delta-delta {' + '    width: 100%;' + '    display: inline-block;' + '}', '.sdd-component {' + '    margin: 0;' + '    border: none;' + '    padding: 0;' + '    display: inline-block;' + '}',

        //controls
        '#sdd-controls {' + '    width: 25%;' + '    float: left;' + '}', '#sdd-controls .control-group {' + '    width: 98%;' + '    margin: 0 2% 5px 0;' + '    padding: 0;' + '}', '#sdd-controls .control-group > * {' + '    display: inline-block;' + '}', '#sdd-controls .changer {' + '    width: 50%;' + '    float: right;' + '    clear: right;' + '}', '#sdd-controls .wc-control-label {', '    text-align: right;' + '    width: 48%;' + '    float: left;' + '    clear: left;' + '}', '#sdd-controls .annote {' + '    width: 98%;' + '    text-align: right;' + '}',

        //chart
        '#sdd-chart {' + '    width: 36%;' + '    margin: 0 2%;' + '}', '.wc-data-mark {' + '    cursor: pointer;' + '}', '.wc-data-mark:hover {' + '    stroke: black;' + '    stroke-width: 3;' + '}', '.regression-note {' +
        //'    font-size: 0.8em;' +
        '    color: #999;' + '}',
    ...
    function safetyDeltaDelta() {
        ...
        var chart = webcharts.createChart(document.querySelector(element).querySelector('#sdd-chart'), syncedSettings, controls);
