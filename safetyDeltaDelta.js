(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, global.safetyDeltaDelta = factory(global.d3, global.webCharts));
    ...
    function syncSettings(settings) {
      ...
      if (!(settings.filters instanceof Array)) settings.filters = typeof settings.filters === 'string' ? [settings.filters] : []; // Handle a string argument to detail.
      ...
      if (settings.visits.hasOwnProperty('baseline')) settings.visits.timepoint1 = settings.visits.baseline; // Map [ visits.comparison ] to [ visits.timepoint1 ] to maintain backwards compatibility.
    ...
    function cleanData() {
      ...
      if (nRemoved > 0) console.warn("".concat(nRemoved, " missing or non-numeric result").concat(nRemoved > 1 ? 's have' : ' has', " been removed.")); //Preserve cleaned data.
    ...
    function trimMeasures() {
      ...
      this.initial_data.forEach(function (d) {
        ...
        d.sdd_measure = d.hasOwnProperty(_this.config.unit_col) ? "".concat(d[_this.config.measure_col], " (").concat(d[_this.config.unit_col], ")") : d[_this.config.measure_col];
      });
    }
    ...
    function checkFilters() {
      ...
      if (this.config.filters) this.config.filters = this.config.filters.filter(function (filter) {
        ...
        if (!variableExists) console.warn(" The [ ".concat(filter.label, " ] filter has been removed because the variable does not exist."));else if (nLevels < 2) console.warn("The [ ".concat(filter.label, " ] filter has been removed because the variable has only one level."));
    ...
    function getMeasures() {
      ...
      this.sdd_measures = this.initial_data[0].hasOwnProperty(this.config.measure_order_col) ? _toConsumableArray(new Set(this.initial_data.map(function (d) {
        return +d[_this.config.measure_order_col];
      })).values()).sort(function (a, b) {
        return a - b;
      }).map(function (value) {
    ...
    function getVisits() {
      ...
      if (this.config.visit_order_col && this.initial_data[0].hasOwnProperty(this.config.visit_order_col)) this.visits = _toConsumableArray(new Set(this.initial_data.map(function (d) {
      ...
      });else this.visits = _toConsumableArray(new Set(this.initial_data.map(function (d) {
    ...
    function initSettings() {
      ...
      this.config.visits.timepoint2 = this.config.visits.timepoint2.length > 0 ? this.config.visits.timepoint2 : [this.visits[this.visits.length - 1]];
    }
    ...
    function initNotes() {
      ...
      this.wrap.insert('p', ':first-child').attr('class', 'record-note').style('text-align', 'center').style('font-weight', 'bold').text('Click a point to see details.'); //Add header element in which to list visits at which measure is captured.
    ...
    function updateVisitControls() {
      ...
      this.config;
      var timepoint1Select = this.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'visits.timepoint1';
      }).select('select');
      timepoint1Select.selectAll('option').filter(function (f) {
        return _this.config.visits.timepoint1.indexOf(f) > -1;
      }).attr('selected', 'selected');
      var timepoint2Select = this.controls.wrap.selectAll('.control-group').filter(function (f) {
        return f.option === 'visits.timepoint2';
      }).select('select');
      timepoint2Select.selectAll('option').filter(function (f) {
    ...
    function getMeasureDetails(pt_data) {
      ...
      var measure_details = d3.nest().key(function (d) {
        ...
        measure_obj.toggle = '+';
        measure_obj.raw = di;
        measure_obj.axisFlag = measure_obj.key == config.measure.x ? 'X' : measure_obj.key == config.measure.y ? 'Y' : '';
      ...
      measure_details = measure_details.map(function (m) {
        return m.values;
      }).sort(function (a, b) {
        if (a.axisFlag == 'X') return -1;else if (b.axisFlag == 'X') return 1;else if (a.axisFlag == 'Y') return -1;else if (b.axisFlag == 'Y') return 1;else if (a.key < b.key) return -1;else if (b.key > a.key) return 1;else return 0;
      });
    ...
    function updateParticipantCount(chart, selector, id_unit) {
      ...
      var currentObs = chart.filtered_data.filter(function (f) {
        return !isNaN(f.delta_x) && f.delta_x !== null && !isNaN(f.delta_y) && f.delta_y !== null;
    ...
    function reset() {
      ...
      this.wrap.select('.record-note').style('text-align', 'center').text('Click a point to see details.');
      this.listing.draw([]);
    ...
    function drawBoxPlot(svg, results, height, width, domain, boxPlotWidth, boxColor, boxInsideColor, fmt, horizontal) {
      ...
      boxplot.append('rect').attr('class', 'boxplot fill').attr('x', box_x).attr('width', box_width).attr('y', box_y).attr('height', box_height).style('fill', boxColor); //draw dividing lines at median, 95% and 5%
    ...
    function addBoxPlots() {
      ...
      ybox.select('g.boxplot').attr('transform', 'translate(' + (this.plot_width + this.config.margin.right / 2) + ',0)'); //X-axis box plot

      var xValues = this.current_data.map(function (d) {
        return d.values.x;
      });
      var xbox = this.svg.append('g').attr('class', 'xMargin');
      drawBoxPlot(xbox, //svg element
      xValues, //values
      1, //height
    ...
    function updateClipPath() {
      ...
      .attr('transform', "translate(-".concat(radius + 1 // translate left circle radius + circle stroke width
      , ",-").concat(radius + 1 // translate up circle radius + circle stroke width
      , ")"));
    }
      ...
      if (this.data.raw.length > 0) {
        ...
        this.tbody.selectAll('tr').style('background', 'none').style('border-bottom', '.5px solid black').each(function (row_d) {
          ...
          var cell = d3.select(this).select('td.spark').classed('minimized', true).text('');
              d3.select(this).select('td.toggle').html('&#x25BD;').style('cursor', 'pointer').style('color', '#999').style('vertical-align', 'middle');
              var width = 100,
              height = 25,
              offset = 4,
          ...
          var x = d3.scale.ordinal().domain(chart.visits).rangeBands([offset, width - offset]); //y-domain includes 99th population percentile + any participant outliers
          ...
          canvas.selectAll('circle').data(overTime).enter().append('circle').attr('class', 'circle outlier').attr('cx', function (d) {
            return x(d[config.visit_col]);
          }).attr('cy', function (d) {
            return y(d[config.value_col]);
    ...
    function addAxisFlag() {
      ...
      ['X', 'Y'].forEach(function (axis) {
        var cell = table.tbody.selectAll('tr').filter(function (d) {
          return d.axisFlag == axis;
        }).select('td.key').text('');
        cell.append('span').attr('class', 'sdd-axisLabel').text(axis + '-axis');
    ...
    function showParticipantDetails(d) {
      ...
      var ul = table.wrap.insert('ul', '*').attr('class', 'pdd-pt-details').style('list-style', 'none').style('padding', '0');
    ...
    function addRegressionLine() {
      if (this.config.add_regression_line) {
        ...
        var arrayData = chart.filtered_data.filter(function (f) {
        ...
        chart.svg.append('path').classed('regressionLine', true).datum([min_xy, max_xy]).attr('d', line).attr('stroke', 'black').attr('stroke-dasharray', '3,5'); //add footnote with R2 and exact calculation

        chart.wrap.select('span.regression-note').remove();
        chart.wrap.append('span').classed('regression-note', true).html('The dashed line shows the result of a simple linear regression. Additional details are shown below. <br> Equation: ' + result.string + '<br> R<sup>2</sup>: ' + d3.format('0.2f')(result.r2));
      }
    }
    ...
    function styles() {
      var styles = ['#safety-delta-delta {' + '    width: 100%;' + '    display: inline-block;' + '}', '.sdd-component {' + '    margin: 0;' + '    border: none;' + '    padding: 0;' + '    display: inline-block;' + '}', //controls
      '#sdd-controls {' + '    width: 25%;' + '    float: left;' + '}', '#sdd-controls .control-group {' + '    width: 98%;' + '    margin: 0 2% 5px 0;' + '    padding: 0;' + '}', '#sdd-controls .control-group > * {' + '    display: inline-block;' + '}', '#sdd-controls .changer {' + '    width: 50%;' + '    float: right;' + '    clear: right;' + '}', '#sdd-controls .wc-control-label {', '    text-align: right;' + '    width: 48%;' + '    float: left;' + '    clear: left;' + '}', '#sdd-controls .annote {' + '    width: 98%;' + '    text-align: right;' + '}', //chart
      '#sdd-chart {' + '    width: 36%;' + '    margin: 0 2%;' + '}', '.wc-data-mark {' + '    cursor: pointer;' + '}', '.wc-data-mark:hover {' + '    stroke: black;' + '    stroke-width: 3;' + '}', '.regression-note {' + //'    font-size: 0.8em;' +
      '    color: #999;' + '}', //listing
      '#sdd-listing {' + '    width: 35%;' + '    float: right;' + '}', '#sdd-listing .wc-table table {' + '    width: 100%;' + '    display: table;' + '}', '#sdd-listing .wc-table th:not(:first-child),' + '#sdd-listing .wc-table td:not(:first-child) {' + '    text-align: right;' + '}', '.sdd-axisLabel{' + 'font-size:75%;' + 'border-radius:0.25em;' + 'padding:.2em .6em .3em;' + 'margin-right:0.4em;' + 'background-color:#5bc0de;' + 'color:white;' + 'font-weight:700;' + '}'];
    ...
    function safetyDeltaDelta() {
      ...
      var controls = webcharts.createControls(document.querySelector(element).querySelector('#sdd-controls'), {
        location: 'top',
        inputs: syncedControlInputs
      });
      var chart = webcharts.createChart(document.querySelector(element).querySelector('#sdd-chart'), syncedSettings, controls); //Define chart callbacks.
      ...
      var listing = webcharts.createTable(document.querySelector(element).querySelector('#sdd-listing'), configuration.listingSettings());
